<template>
  <div class="onepiano-app-wrap">
    <div class="piano-wrap" v-show="showPianoKeyboard">
      <div class="piano-band"></div>
      <div class="piano-key-wrap">
        <div class="piano-key wkey" v-for="(note, index) in Notes" :key="'white'+index" :data-keyCode = "note.keyCode" v-if="note.type=='white'" @click="clickPianoKey($event, note.keyCode)">
          <div class="keytip">
    ...
    <div class="piano-options">
      ...
      <div class="option-item">
        <input type="checkbox" id="pianokeyboard" v-model="showPianoKeyboard">
        <label for="pianokeyboard">显示键盘</label>
      </div>
  ...
  methods: {
    ...
    bindKeyBoradEvent() {
      ...
      document.addEventListener('keydown', (e) => {
        ...
        if (vm.DEV) console.log('[ + ]keydown-keycode: ', key, keyCode);
        // 按住空格键，则启用黑色按键
        if (keyCode == 32 || key == ' ') {
        ...
        if (vm.enableBlackKey) {
          ...
          let pressedNote =  vm.enableBlackKey ? this.getNoteByKeyCode(keyCode) : this.getNoteByKey(key)
          if (pressedNote) {
